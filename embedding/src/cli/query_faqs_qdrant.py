...
def load_domain_keywords():
    ...
    # Since outbound_keywords.json has been removed, return empty sets
    # Only keep xuno domain keywords if they exist elsewhere, otherwise empty
    outbound = set()
    xuno_domain = set()
    return outbound, xuno_domain
...
def query_faqs(user_query, qdrant_client, top_k=5, min_similarity_threshold=0.4):
    ...
    out_of_domain_keywords, xuno_domain_keywords = load_domain_keywords()
    ...
    for hit in search_result.points:
        ...
        query_words_filtered = [word for word in user_query_lower.split() if word not in stop_words and len(word) > 2]
        faq_words_filtered = [word for word in faq_question_lower.split() if word not in stop_words and len(word) > 2]

        # Count meaningful word matches
        query_set = set(query_words_filtered)
        faq_set = set(faq_words_filtered)
        ...
        # Calculate domain relevance (outbound keywords have been removed)
        query_domain_matches = query_words.intersection(xuno_domain_keywords)
        faq_words = set(faq_question_lower.split())
        faq_domain_matches = faq_words.intersection(xuno_domain_keywords)
        ...
        greeting_query = bool(greeting_terms.intersection(query_words))
        greeting_answer = ("greetings" in faq_question_lower or "hello" in faq_question_lower or "hi" in faq_question_lower or "hey" in faq_question_lower or "good" in faq_question_lower)
        ...
        # Calculate final similarity (outbound detection has been removed)
        final_similarity = (similarity_score * 0.5) + (orthographic_similarity * 0.2) + (semantic_relevance_boost * 0.3)
        final_similarity += boost * 0.05  # Reduce impact of generic boosts
